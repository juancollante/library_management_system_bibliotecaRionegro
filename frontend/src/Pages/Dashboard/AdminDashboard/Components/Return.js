...
function Return() {
    ...
    return (
        <div>
            ...
            <p className="dashboard-option-title">Prestados</p>
                        ...
                        allTransactions?.filter((data)=>{
                            ...
                            return (
                                <tr key={index}>
                                    ...
                                    <td><button onClick={()=>{returnBook(data._id,data.borrowerId,data.bookId,(Math.floor(( Date.parse(moment(new Date()).format("MM/DD/YYYY")) - Date.parse(data.toDate) ) / 86400000)))}}>Devolver</button></td>
                                </tr>
                            )
                        })
                    }
                </table>
                <p className="dashboard-option-title">Reservados</p>
            <table className="admindashboard-table">
                    <tr>
                        <th>Nombre del libro</th>
                        <th>Nombre del prestatario</th>
                        ...
                        allTransactions?.filter((data)=>{
                            ...
                            return (
                                <tr key={index}>
                                    ...
                                    <td>{data.fromDate}</td>
                                    <td>{data.toDate}</td>
                                    <td><button onClick={()=>{convertToIssue(data._id)}}>Convertir</button></td>
                                </tr>
                            )
                        })
                    }
